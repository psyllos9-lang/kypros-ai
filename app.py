...
translations = {
    "Ελληνικά": {
        ...
        "book_desc": "Επίλεξε Club, γήπεδο και ώρα:",
        ...
        "already_booked": "⚠️ Η Ώρα αυτή είναι κρατημένη από:",
    ...
    "English": {
        ...
        "book_desc": "Select Club, court, and time slot:",
        ...
        "already_booked": "⚠️ This slot is already booked by:",
        "match_title": "👥 Find a Player (Matchmaking)",
    ...
    "Русский": {
        ...
        "book_desc": "Выберите клуб, корт и время:",
        ...
        "already_booked": "⚠️ Это время уже забронировано:",
    ...
    "עברית": {
        ...
        "book_desc": "בחר מועדון, מגרש ושעה:",
        ...
        "already_booked": "⚠️ שעה זו כבר תפוסה על ידי:",
...
if "open_matches" not in st.session_state:
    st.session_state.open_matches = [
        {"Club": "PadelPro Limassol", "Court": "Court 1 (Indoor)", "Time": "19:00", "Level": "Intermediate (3.0-3.5)", "Player": "Μιχάλης", "Needed": "2 παίκτες"},
        {"Club": "Limassol Padel Club", "Court": "Court 3 (Panoramic)", "Time": "20:30", "Level": "Advanced (4.0+)", "Player": "Александр", "Needed": "1 player"}
    ]
...
with tab1:
    ...
    slot_key = f"{selected_club} - {selected_court} @ {selected_time}"
...
with tab2:
    ...
    with st.form("match_form", border=False):
        m_club = st.selectbox("Club:", list(clubs_data.keys()))
        m_court = st.selectbox(t['court_label'], clubs_data[m_club])
        m_time = st.selectbox(t['time_label'], time_slots)
        m_level = st.selectbox(t['level_label'], ["Beginner (1.0-2.0)", "Intermediate (2.5-3.5)", "Advanced (4.0+)"])
        m_player = st.text_input(t['name_label'], placeholder="π.χ. Νίκος")
...
with tab3:
    ...
    if st.session_state.bookings:
        df_book = pd.DataFrame([{"Slot & Court": k, "Player": v} for k, v in st.session_state.bookings.items()])
