...
# --- FUTURISTIC CYBERPUNK & CLEAN UI STYLING ---
st.markdown("""
    <style>
    @import url('https://fonts.googleapis.com/css2?family=Orbitron:wght@600;800;900&family=Plus+Jakarta+Sans:wght@400;500;600;700&display=swap');
...
# --- RENDER FUTURISTIC HERO BANNER ---
st.markdown("""
    <div class="hero-banner">
        <div class="hero-top">
            <div>
                <h1 class="hero-title">⚡ E-Hub Cyprus</h1>
                <p class="hero-subtitle">Real-Time EV Charging & Route Intelligence Platform</p>
            </div>
            <div>
                <span class="live-badge">🟢 LIVE 24/7 SYNC</span>
            </div>
        </div>
    </div>
""", unsafe_allow_html=True)
...
with st.sidebar:
    ...
    st.markdown("""
        ...
        * **Status:** 100% Real-time Live
    """)
    st.markdown("---")
    # Live feed indicator to prove automatic updates
    st.success("🔄 Live API Stream: Connected (Auto-refresh active)")

# --- MODE 1: INTERACTIVE MAP ---
if app_mode == "🗺️ Live Network Map":
    st.markdown("### 📍 Live Interactive Infrastructure Map")
    st.write("Real-time connector status tracking across Cyprus charging corridors.")
    ...
    # --- CLEAN MAP (Using CartoDB Positron with attribution minimized to eliminate unwanted symbols) ---
    ...
    for idx, row in df_filtered.iterrows():
        ...
        popup_html = f"""
            <div style="font-family: 'Plus Jakarta Sans', sans-serif; min-width: 180px;">
                <b style="font-size: 14px; color: #0f172a;">{row['Name']}</b><br>
    ...
    st.markdown("### 📋 Live Station Telemetry")
    st.dataframe(df_filtered[["Name", "City", "Operator", "Type", "Status"]], use_container_width=True)

# --- MODE 2: CALCULATOR ---
else:
    st.markdown("### 🔋 Smart Energy & Cost Calculator")
    st.write("Calculate live charging session expenses and duration based on current Cypriot electricity tariffs.")
