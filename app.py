...
# Page Configuration
st.set_page_config(
    page_title="Έι Άι // Ultimate Edition",
    page_icon="⚡",
    layout="centered",
    initial_sidebar_state="expanded"
)

# Futuristic Cyber-Pro Styling
st.markdown("""
    <style>
...
st.markdown("""
    <div class="brand-container">
        ...
        <div class="brand-text">
            <h1>Έι Άι</h1>
            <p>Ultimate Unlimited Edition // 2026</p>
        </div>
    </div>
""", unsafe_allow_html=True)
...
with st.sidebar:
    ...
    app_mode = st.radio("Επιλογή Λειτουργίας:", ["💬 Συνομιλία (Chat)", "🎨 Δημιουργία Εικόνας (Image Gen)"])
    
    st.info("💡 Εντελώς δωρεάν, χωρίς API keys, χωρίς όρια χρήσης.")
...
if app_mode == "🎨 Δημιουργία Εικόνας (Image Gen)":
    ...
    st.write("Γράψε τι θέλεις να δημιουργήσει η τεχνητή νοημοσύνη (στα αγγλικά ή ελληνικά):")
    ...
    if st.button("Δημιουργία Εικόνας", use_container_width=True):
        if img_prompt:
            with st.spinner("Δημιουργείται η εικόνα σου..."):
                try:
                    # Χρήση του Pollinations AI (100% δωρεάν, απεριόριστο API για εικόνες)
                    ...
                    if response.status_code == 200:
                        ...
                        st.image(image, caption=f"Prompt: {img_prompt}", use_container_width=True)
                    else:
                        st.error("⚠️ Αποτυχία φόρτωσης εικόνας. Δοκίμασε ξανά.")
...
# Mode 2: Chat Assistant
    ...
    with st.spinner("Φόρτωση συστήματος AI..."):
        chatbot_pipeline = load_chat_model()

    # Modern Cypriot Style System Prompt (Ο τρόπος που μιλάει ο κόσμος 15-50 ετών στην Κύπρο σήμερα)
    system_instruction = (
        "Είσαι ένας έξυπνος και σύγχρονος ψηφιακός βοηθός. "
