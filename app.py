...
st.markdown("""
    <div class="pro-header">
        ...
        <div>
            <span class="status-pill">● LIVE API SYNC</span>
        </div>
    </div>
""", unsafe_allow_html=True)

# --- FETCH ALL LIVE STATIONS WITH FULL TELEMETRY FROM OPEN CHARGE MAP ---
@st.cache_data(ttl=300)
def fetch_all_cyprus_chargers_live():
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            for item in data:
                ...
                # Extract Power / Speed & Availability telemetry
                connections = item.get("Connections", [])
                speed_type = "AC Standard (22kW)"
                max_kw = 0
    ...
    # Fallback complete dataset if API is rate-limited
    return pd.DataFrame({
...
chargers_db = fetch_all_cyprus_chargers_live()
...
with st.sidebar:
    ...
    st.markdown(f"""
        * **Live Nodes Found:** {len(chargers_db)} Stations
        * **Coverage:** Cyprus Island-wide
        * **Telemetry:** Speeds & Status Live
    """)

# --- MODE 1: LIVE MAP ---
if app_mode == "🗺️ Live National Map":
    st.markdown("### 📍 Comprehensive Cyprus EV Network Map")
    st.write("Real-time telemetry tracking of all public nodes, speeds, and live availability.")
    ...
    with m3:
        st.markdown(f"""
            <div class="metric-card">
                ...
                <p class="metric-label">Live Telemetry</p>
            </div>
        """, unsafe_allow_html=True)
