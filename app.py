...
st.markdown("""
    <div class="chat-header">
        <h1>Έι Άι 🇨🇾</h1>
        <p>Ο πλήρως εξελιγμένος ψηφιακός βοηθός σου με ζωντανή πρόσβαση στο internet</p>
    </div>
""", unsafe_allow_html=True)
...
# Strict Natural & Modern Cypriot System Prompt with Real-Time Web Access
CYPRIOT_SYSTEM_PROMPT = (
    "Εσείσαι ένας σύγχρονος, πανέξυπνος, φιλικός και ντόπιος Κύπριος ψηφιακός βοηθός με πλήρη πρόσβαση στο διαδίκτυο σε πραγματικό χρόνο. "
    ...
    "Απαγορεύεται αυστηρά να λες ότι δεν έχεις πρόσβαση στο internet· όταν σε ρωτούν για νέα, ειδήσεις, γεγονότα ή πληροφορίες, χρησιμοποιείς πάντα την αναζήτηση για να βρεις τα πιο πρόσφατα δεδομένα και τα παρουσιάζεις με απόλυτη φυσικότητα στα κυπριακά."
)

# Initialize Session State with a natural greeting
if "messages" not in st.session_state:
    st.session_state.messages = [
        {"role": "assistant", "content": "Έλα κοπέλι! Τι χαμπάρια; Είμαι ενήμερος για τα πάντα πλέον, ρωτά με ό,τι θες!"}
    ]
...
if prompt := st.chat_input("Λάλε μου κάτι..."):
    ...
    with st.chat_message("assistant"):
        ...
        message_placeholder.markdown("*Ψάχνει τα πάντα...*")
        
        try:
            # We initialize the model with Google Search Tool enabled for live web results
